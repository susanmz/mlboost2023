...
ngb = NGBRegressor(n_estimators=500).fit(X_train, Y_train, X_test, Y_test)
Npoints = 101
...
test_mse_list = []
colors = ['r', 'g', 'b', 'k', 'c']
for i, max_iter in enumerate(range(100,600,100)):
    Y_preds = ngb.predict(stroke_X_test, max_iter=max_iter)
    Y_dists = ngb.pred_dist(stroke_X_test, max_iter=max_iter)
    test_NLL = -Y_dists.logpdf(stroke_Y_test).mean()
    test_MSE = mean_squared_error(Y_preds, stroke_Y_test, squared=False)
    test_nll_list.append(test_NLL)
    test_mse_list.append(test_MSE)
    print("Max iter", max_iter, ', Test NLL', np.around(test_NLL,3), ', Test RMSE', np.around(test_MSE,3))
    plt.plot(norm_percent, Y_preds, colors[i], label='Max Iter: '+str(max_iter))
...
ax1.plot(range(100,600,100), test_nll_list)
ax1.set_xlabel("Iterations")
ax1.set_ylabel("NLL Test Loss")
ax1.set_title("NLL")
ax2.plot(range(100,600,100), test_mse_list)
ax2.set_xlabel("Iterations")
ax2.set_ylabel("RMSE Test Loss")
ax2.set_title("RMSE")
plt.show()